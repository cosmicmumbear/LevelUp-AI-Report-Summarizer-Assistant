...
class AzureConfig:
    ...
    CUSTOM_VISION_PREDICTION_RESOURCE_ID = os.getenv(
    ...
    OPENAI_API_VERSION = "2024-02-15-preview"  # Standardowa wersja
    OPENAI_DEPLOYMENT_NAME = os.getenv("AZURE_OPENAI_DEPLOYMENT")  # np. gpt-4o
    ...
    def get_custom_vision_training_client():
        ...
        if (
            not AzureConfig.CUSTOM_VISION_TRAINING_KEY
            or not AzureConfig.CUSTOM_VISION_TRAINING_ENDPOINT
        ):
            raise ValueError("Missing keys VISION in .env file!")
        credentials = ApiKeyCredentials(
            in_headers={"Training-key": AzureConfig.CUSTOM_VISION_TRAINING_KEY}
        )
        client = CustomVisionTrainingClient(
            AzureConfig.CUSTOM_VISION_TRAINING_ENDPOINT, credentials
        )
    ...
    def get_custom_vision_prediction_client():
        ...
        if (
            not AzureConfig.CUSTOM_VISION_PREDICTION_KEY
            or not AzureConfig.CUSTOM_VISION_PREDICTION_ENDPOINT
        ):
            raise ValueError("Missing keys VISION in .env file!")

        credentials = ApiKeyCredentials(
            in_headers={"Prediction-key": AzureConfig.CUSTOM_VISION_PREDICTION_KEY}
        )
        client = CustomVisionPredictionClient(
            AzureConfig.CUSTOM_VISION_PREDICTION_ENDPOINT, credentials
        )
